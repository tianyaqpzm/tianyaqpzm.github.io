...
1、put方法：计算key的hashCode(),然后对该哈希码值**进行哈希**，然后把哈希值和（数组长度-1） 进行按位与操作，得到存储的数组下标，2、如果该位置没有链表节点，直接放入，3、如果有节点，就对链表进行遍历查询是否有hash、key和要放入对节点相同的节点，如果有，就替换该节点的value，如果没有就创建节点，并插入到链表表头（头插法）
